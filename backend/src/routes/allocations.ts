...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!buildingId && !roomId) {
      return res
        .status(400)
        .json({ error: "Either buildingId or roomId must be provided" });
    }
    ...
    try {
      ...
      if (roomId) {
        ...
        if (room.branchAllocated) {
          ...
          // Room is unallocated - check which branches are available
          ...
          const allocatedRoomsInOtherBuildings = await prisma.room.findMany({
            where: {
              buildingId: { not: room.buildingId },
          ...
          eligibleBranches = allBranches.filter(
            (branch) =>
              branchesWithStudentsSet.has(branch) &&
              !allocatedBranchesInOtherBuildings.has(branch)
          );
        }
      } else {
    ...
    try {
      ...
      if (version !== undefined && allocation.version !== version) {
        return res.status(409).json({
          message:
            "Seat has been modified by another user. Please refresh and try again.",
          currentSeat: {
            id: allocation.id,
      ...
      if (error.code === "P2025") {
        return res.status(409).json({
          message:
            "Seat or room has been modified by another user. Please refresh and try again.",
        });
      }
